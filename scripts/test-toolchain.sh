...
if [[ "$PROFILE" == "drawing" ]]; then
  ...
  grep -Eiq '^[[:space:]]*freecad[[:space:]]' "${ACK_ROOT}/FREECAD_PACKAGES.txt"
  grep -A2 '^Inkscape  grep -A2 '^drawsvg$' "${ACK_TXT}" | grep -Eq '^  Version[[:space:]]+:[[:space:]]+[^[:space:]].*$'
  ...
  grep -A2 '^Inkscapeif [[ "$PROFILE" == "full" ]]; then
...
echo "Smoke test passed for ${PROFILE} runtime."
 "${ACK_TXT}" | grep -Eq '^  Package[[:space:]]+:[[:space:]]+[^[:space:]].*  grep -A2 '^drawsvg$' "${ACK_TXT}" | grep -Eq '^  Version[[:space:]]+:[[:space:]]+[^[:space:]].*$'
  if grep -A2 '^Inkscape$' "${ACK_TXT}" | grep -q 'not installed in this profile'; then
    echo "ERROR: drawing-profile acknowledgments report Inkscape as not installed." >&2
    exit 1
  fi
...
echo "Smoke test passed for ${PROFILE} runtime."
  ...
  grep -A3 '^FreeCAD  grep -A2 '^drawsvg$' "${ACK_TXT}" | grep -Eq '^  Version[[:space:]]+:[[:space:]]+[^[:space:]].*$'
  if grep -A2 '^Inkscape$' "${ACK_TXT}" | grep -q 'not installed in this profile'; then
    echo "ERROR: drawing-profile acknowledgments report Inkscape as not installed." >&2
    exit 1
  fi
...
echo
echo "Smoke test passed for ${PROFILE} runtime."
 "${ACK_TXT}" | grep -Eq '^  Version[[:space:]]+:[[:space:]]+[^[:space:]].*  grep -A2 '^drawsvg$' "${ACK_TXT}" | grep -Eq '^  Version[[:space:]]+:[[:space:]]+[^[:space:]].*$'
  if grep -A2 '^Inkscape$' "${ACK_TXT}" | grep -q 'not installed in this profile'; then
    echo "ERROR: drawing-profile acknowledgments report Inkscape as not installed." >&2
    exit 1
  fi
...
echo
echo "Smoke test passed for ${PROFILE} runtime."
 "${ACK_TXT}" | grep -q 'not installed in this profile'
  grep -A3 '^FreeCADif [[ "$PROFILE" == "full" ]]; then
  grep -Eiq '^pybosl2[[:space:]]' "${PYTHON_INVENTORY}"
  grep -Eiq '^Shapely[[:space:]]' "${PYTHON_INVENTORY}"
fi
printf 'Acknowledgments: %s and %s\n' \
...
echo "Smoke test passed for ${PROFILE} runtime."
 "${ACK_TXT}" | grep -q 'not installed in this profile'
  grep -A2 '^drawsvgif [[ "$PROFILE" == "full" ]]; then
  grep -Eiq '^pybosl2[[:space:]]' "${PYTHON_INVENTORY}"
  grep -Eiq '^Shapely[[:space:]]' "${PYTHON_INVENTORY}"
fi
printf 'Acknowledgments: %s and %s\n' \
...
echo "Smoke test passed for ${PROFILE} runtime."
 "${ACK_TXT}" | grep -q 'not installed in this profile'
  grep -Fxq 'not installed in this profile' "${ACK_ROOT}/FREECAD_PACKAGES.txt"
fi
if [[ "$PROFILE" == "full" ]]; then
  grep -Eiq '^pybosl2[[:space:]]' "${PYTHON_INVENTORY}"
  grep -Eiq '^Shapely[[:space:]]' "${PYTHON_INVENTORY}"
