...
echo "Smoke test passed for ${PROFILE} runtime."
 "${ACK_TXT}" | grep -Eq '^  Package[[:space:]]+:[[:space:]]+[^[:space:]].*if [[ "$PROFILE" == "full" ]]; then
  grep -Eiq '^pybosl2[[:space:]]' "${PYTHON_INVENTORY}"
  grep -Eiq '^Shapely[[:space:]]' "${PYTHON_INVENTORY}"
fi
printf 'Acknowledgments: %s and %s\n' \
...
echo "Smoke test passed for ${PROFILE} runtime."
  ...
  grep -A2 '^FreeCADif [[ "$PROFILE" == "full" ]]; then
  grep -Eiq '^pybosl2[[:space:]]' "${PYTHON_INVENTORY}"
  grep -Eiq '^Shapely[[:space:]]' "${PYTHON_INVENTORY}"
fi
printf 'Acknowledgments: %s and %s\n' \
...
echo
echo "Smoke test passed for ${PROFILE} runtime."
 "${ACK_TXT}" | grep -Eq '^  Version[[:space:]]+:[[:space:]]+[^[:space:]].*if [[ "$PROFILE" == "full" ]]; then
  grep -Eiq '^pybosl2[[:space:]]' "${PYTHON_INVENTORY}"
  grep -Eiq '^Shapely[[:space:]]' "${PYTHON_INVENTORY}"
fi
printf 'Acknowledgments: %s and %s\n' \
...
echo "Smoke test passed for ${PROFILE} runtime."
  ...
  if grep -A2 '^Inkscapeif [[ "$PROFILE" == "full" ]]; then
  grep -Eiq '^pybosl2[[:space:]]' "${PYTHON_INVENTORY}"
  grep -Eiq '^Shapely[[:space:]]' "${PYTHON_INVENTORY}"
fi
printf 'Acknowledgments: %s and %s\n' \
...
echo
echo "Smoke test passed for ${PROFILE} runtime."
 "${ACK_TXT}" | grep -q 'not installed in this profile'; then
    echo "ERROR: drawing-profile acknowledgments report Inkscape as not installed." >&2
    exit 1
  fi
  if grep -A2 '^FreeCADif [[ "$PROFILE" == "full" ]]; then
  grep -Eiq '^pybosl2[[:space:]]' "${PYTHON_INVENTORY}"
  grep -Eiq '^Shapely[[:space:]]' "${PYTHON_INVENTORY}"
fi
printf 'Acknowledgments: %s and %s\n' \
